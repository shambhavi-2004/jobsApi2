...
https://chatgpt.com/c/ecbed278-0313-4bd3-921d-35f8ae22d23c
no external application mist access server (except our frontend)
...
-this is what the default users is sent to the login function looks like
...
- make sure email and password match your test user
  -react gives put a request to the login route ,so this type of an user is already saved in our database(must be made manually by us)=>thi suser will be having restricted functionality so that the outside person doesnt modify the test user credentials (they will always be the same) and other functionality
...
-there must be mw as one user must not be able to update credentials of other user
